...
exports.collectGraphemes = collectGraphemes;
...
let TAIL_E;
{
  let fill = (/** @type {Uint8Array} */t, /** @type {number} */lo, /** @type {number} */from, /** @type {number} */to, /** @type {number} */cat) => {
    let end = to - lo + 1,
      len = t.length;
  ...
  let starts = new Uint32Array(320),
    ends = new Uint32Array(320),
    n = 0;
  (0, _core.decodeUnicodeData)(_grapheme_data.grapheme_data, _grapheme_data.grapheme_cats, (from, to, cat) => {
...
function catRare(cp) {
  ...
  return (0, _core.findUnicodeRangeCategory)(cp, TAIL_S, TAIL_E);
}
...
const PAIR = Uint8Array.from(_grapheme_data.grapheme_pairs);
 ...
 */
function collectGraphemes(input) {
  /** @type {string[]} */
  let result = [];
  let len = input.length;
